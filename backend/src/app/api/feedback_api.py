...
@feedback_api.route('/delete/<string:id>' , methods=['DELETE'])
@token_required_role_admin
...
@feedback_api.route('/update' , methods=['POST'])
@token_required
def update_feedback(current_user):
    try:
        data = request.get_json()
        id = data['id']
...
@feedback_api.route('/delete_your_feedback/<string:id>' , methods=['DELETE'])
@token_required
def delete_your_feedback(current_user,id):
    try:
        feedback = Feedback.query.filter_by(id=id,user_id=current_user.id).first()
        if not feedback:
            return custom_response({'message' : 'No feedback found!'},404)
