...
class Feedback(Base):
    ...
    def get_feedback_by_hotel_id(hotel_id):
        # return Feedback.query.filter_by(hotel_id=hotel_id).join(User,User.id == Feedback.user_id).first()
        # return Feedback.query(Feedback,User).join(User).filter(Feedback.hotel_id ==hotel_id).all()
